...
/*
 * Assignment metadata
 * Name and NetID: Emily Liu (eyl48)
...
public class CMSu {
    ...
    public boolean hasConflict(Student student) {
        for(int i = 0; i < nCourses - 1; i++) {
            for(int j = i + 1; j < nCourses; j++) {
                if(courses[i].hasStudent(student) && courses[j].hasStudent(student)) {
     ...
     */
    private int totalCredits(Student student){
        int check = 0;
        for (int i = 0; i < nCourses; i++){
            if (courses[i].hasStudent(student)){
                check += courses[i].credits();
            }
        }
        return check;
    }
    public boolean checkCreditConsistency() {
        ...
        for (int i = 0; i < nStudents; i++){
            if (students[i].credits() == totalCredits(students[i])){
                result = true;
            }
        }
        return result;
