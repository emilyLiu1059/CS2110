...
if(isActive) {
    boolean a=target.checkHit(e.getX(), e.getY());
    if (a) {
        setScore(getScore() + 1);
        repaint();
    }
    ...
    private static class Target {
        ...
        void paintDot(Graphics g) {
            ...
            if(isHit){
                ...
                g.fillOval(x, y, 1*radius, 1*radius);
            }else{
        ...
        boolean checkHit(int cx, int cy) {
            ...
            double q=Math.sqrt(Math.pow((double)cx-(double)x,2)+Math.pow((double)cy-(double)y,2));
            if(q<=(double)radius&&(!isHit)){
                isHit = true;
                return true;
            }else{
                return false;
